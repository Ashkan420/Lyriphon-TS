...
11. Translate for meaning, tone, and emotional intent — NOT word-for-word literal accuracy. The result should read naturally to a native speaker of the target language while preserving the original meaning.
12. Song titles, recurring nicknames, catchphrases, and iconic proper nouns should usually remain untranslated, unless a natural translation is clearly preferable in the target language.
13. Maintain consistency throughout the entire song. If a phrase is translated one way, keep the same translation whenever it appears again.
14. Preserve repetitions exactly. If the same source line appears multiple times, translate it the same way unless context clearly requires otherwise.
15. Avoid overly colloquial slang unless the original lyric is clearly colloquial.
16. Words, phrases, names, and expressions that are already written in the target language should normally remain unchanged. Do NOT translate them into another wording of the same language.
